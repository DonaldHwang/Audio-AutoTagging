...
class AudioTaggingPreProcessing(object):
    ...
    def get_Mels(config):
        ...
        if AudioTaggingPreProcessing.__melBank__ is None:
            AudioTaggingPreProcessing.__melBank__ = librosa.filters.mel(sr=config.resampling_rate,
                                                                        n_fft=config.n_fft,
    ...
    @staticmethod
    def read_audio(config, pathname, fixedLength=False):
        '''
        ...
        # Resampling
        if config.resampling_rate != sr:
            audio = librosa.resample(audio, sr, config.resampling_rate)

        # Fix length
        if fixedLength:
            if audio.shape[0] < config.max_length:  # if too short, repeat many times
                times = math.floor(config.max_length / audio.shape[0])

                tmpAudio = np.array([])
                for i in range(times):
                    tmpAudio = np.append(tmpAudio, audio)

                if tmpAudio.shape[0] < config.max_length:  # Pad with silence on both ends
                    pad_length = config.max_length - tmpAudio.shape[0]
                ...
                res = audio.shape[0] - config.max_length
                start = math.floor(np.random.rand() * res)
                audio = audio[start : start + config.max_length]
    ...
    @staticmethod
    def audio_to_spectrogram(config, audio):
        '''
        ...
        if config.useMel:
            ...
            mel = AudioTaggingPreProcessing.get_Mels(config)  # Mels are stored as static variable to speed up the process
