...
class AudioTaggingPreProcessing(object):
    ...
    def audio_to_spectrogram(audio, config):
        ...
        if config.use_mels:
            ...
            mel = AudioTaggingPreProcessing.get_Mels(config.resampling_rate, config.n_fft, config.n_mels, config.fmin, config.fmax)  # Mels are stored as static variable to speed up the process
    ...
    def show_spectrogram_batch(batch, config, gridSize=4,
        ...
        for j, i in enumerate(idx):
            ...
            tmpSpec = specs[i, :, :, :].detach().numpy().squeeze()
            if config.use_mels:
                ...
                librosa.display.specshow(tmpSpec, y_axis='log', x_axis='time', sr=config.resampling_rate,
                                         hop_length=config.hop_length)
    ...
    @staticmethod
    def show_waveforms_from_audio_batch(batch, config, gridSize=4,
                             saveFig=True, saveWavs=True, filepath='', filename='Output_batch'):
        '''
        Shows time domain plot AND saves the wave file to disk. This one uses audio data and not the fanme.

        Args:
            batch - a batch dictionary with fields 'audio', 'labels', 'fname'
            config - configuration dictionary
            gridSize - number of plots, in case that the batch is too big
            saveFig - flag for saving the plots to disk
            saveWavs - Save wav files to disk.
            filepath - path to the directory where the plot is saved
            filename - name for the plot
        '''

        audio, labels, fnames = batch['audio'], batch['labels'], batch['fname']
        ...
        for j, i in enumerate(idx):
            ...
            fname = fnames[j]
            tmpAudio = audio[j,:].numpy().squeeze()
            librosa.display.waveplot(tmpAudio, sr=config.resampling_rate)

            plt.title('%d %s' % (j, fname.split('.')[0].split('/')[-1]))

            if saveWavs:
                librosa.output.write_wav(os.path.join(filepath, filename + ('_%d' % (j) + '.wav')), tmpAudio,
                                         config.resampling_rate)

        if saveFig:
            plt.savefig(os.path.join(filepath, filename + ('_waveforms.png')))
        plt.show()
    @staticmethod
